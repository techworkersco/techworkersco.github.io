...
# 1) Import complete archives, from hardcoded Mailchimp URL:
# $ ./scripts/import_legacy_issues.rb
#
# 2) As a tool manually extract sections from an archive URL based on a search
# string, convert them to markdown, and copy the contents to the clipboard.
# (This is useful for manually importing sections that might are missed in the
# bulk import mode.)
# $ ./scripts/import_legacy_issues.rb <url-to-issue> <"string to search for">
...
url = "https://us11.campaign-archive.com/?u=194e57c175176cfd13007a197&id=35d53a2d8d"
search_str =  "SONG OF THE WEEK"
html_doc = Nokogiri::HTML(open(url))
match = html_doc.css('.mcnTextContent').select { |block| block.inner_text.include?(search_str) }.first
puts to_markdown(match.inner_html)
